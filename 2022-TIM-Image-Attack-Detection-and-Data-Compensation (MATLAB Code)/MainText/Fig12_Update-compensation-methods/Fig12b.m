...
figure(22);
title('\tau = 4 - Multi-update', 'FontSize',12);
xlabel('Time(s)', 'FontSize', 10);
ylabel('(rad)', 'FontSize', 10);
box on;
p3 = plot(x, y11, 'c-', x, y12, 'm-', x, y13, 'g-', 'LineWidth', 2); hold on;
p4 = plot(x, y14, 'k-', x, y15, 'b-', x16, y16, 'r-', 'LineWidth', 2); hold on;

l2 = legend(p3, '\rho = 15%', '\rho = 20%', '\rho = 25%', 'Location', 'northwest');
axis([0 time -0.2 0.2]);
axesNew = axes('position',get(gca,'position'), 'visible', 'off');
legend(axesNew, p4, '\rho = 30%', '\rho = 35%', '\rho = 40%', 'Location', 'southwest');
hold on;
axis([0 time -0.2 0.2]);
axesNew = axes('position',get(gca,'position'), 'visible', 'off');
