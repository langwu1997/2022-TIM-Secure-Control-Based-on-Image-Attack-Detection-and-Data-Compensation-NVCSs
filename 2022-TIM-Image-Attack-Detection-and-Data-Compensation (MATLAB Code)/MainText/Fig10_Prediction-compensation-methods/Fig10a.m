...
% box on;
% p1 = plot(x, y1, 'k-', x, y2, 'r-', x, y3,'g-', x4, y4, 'b-',x5, y5,'c-', 'LineWidth',2); hold on;
% l1 = legend(p1, '\rho = 5%', '\rho = 10%', '\rho = 15%', '\rho = 20%', 'Location', 'northwest');
% axis([0 time -0.2 0.2]);
% hold on;
% 
% figure(2);
% title('\tau = 4 - Multi-step', 'FontSize',12);
% xlabel('Time(s)','FontSize',10);
% ylabel('(rad)','FontSize',10);
% box on;
% p3 = plot(x, y7, 'c-', x, y8, 'm-', x, y9, 'g-', 'LineWidth', 2); hold on;
% p4 = plot(x, y10, 'k-', x, y11, 'r-', x12, y12, 'b-', 'LineWidth', 2); hold on;
% l2 = legend(p3, '\rho = 5%', '\rho = 10%', '\rho = 15%', 'Location', 'northwest');
% axis([0 time -0.2 0.2]);
% axesNew = axes('position',get(gca,'position'), 'visible', 'off');
% legend(axesNew, p4, '\rho = 20%', '\rho = 25%', '\rho = 30%', 'Location', 'best');
% hold on;
% 
% figure(33);
% plot(x15, y15, 'g-',  x, y3,'r-', x, y9, 'b-', 'LineWidth',1); 
% legend('Tradictional', 'Single-step', 'Multi-step', 'Location','northwest');
% axis([0 time -0.2 0.2]);
...
% figure(3);
% plot(x4, y4,'r-', x, y10, 'b-', 'LineWidth',2); 
% legend('Single-step', 'Multi-step', 'Location','northwest');
% axis([0 time -0.2 0.2]);
% title('\tau = 4 & \rho = 20%', 'FontSize',12);
% xlabel('Time(s)','FontSize',12); 
% ylabel('(rad)','FontSize',12); 
% hold on;
% 
% figure(4);
% plot(x5, y5, 'r-', x, y11, 'b-', 'LineWidth',2);
% legend('Single-step', 'Multi-step', 'Location','northwest');
% axis([0 time -0.2 0.2]);
% title('\tau = 4 & \rho = 25%', 'FontSize',12);
% xlabel('Time(s)','FontSize',12); 
% ylabel('(rad)','FontSize',12); 
% hold on;

% figure(5);
% plot(x6, y6,'r-', x12, y12, 'b-', 'LineWidth',2); 
% legend('Single-step', 'Multi-step', 'Location','northwest');
% axis([0 time -0.2 0.2]);
% title('\tau = 4 & \rho = 30%', 'FontSize',12);
% xlabel('Time(s)','FontSize',12); 
% ylabel('(rad)','FontSize',12); 
% hold on;
...
figure(6);
plot(x, y13,'r-', x14, y14, 'b-', 'LineWidth',2); 
legend('\rho = 5%','\rho = 10%', 'Location','northwest');
axis([0 time -0.2 0.2]);
%title('\tau = 4 - Tradictional', 'FontSize',12);
xlabel('Time(s)','FontSize',12); 
ylabel('(rad)','FontSize',12); 
hold on;

% figure(7);
% plot(x, y13,'r-', x, y1, 'g-', x, y7, 'b-', 'LineWidth',2); 
% legend('Tradictional', 'Single-step', 'Multi-step', 'Location','northwest');
% title('\tau = 4 & \rho = 10%', 'FontSize',12);
% xlabel('Time(s)','FontSize',12); 
% ylabel('(rad)','FontSize',12); 
% axis([0 time -0.2 0.2]);
% hold on;